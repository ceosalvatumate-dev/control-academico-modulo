...
function RequireAuth({ children }) {
  ...
  const isAuthed = !!window.__firebaseUser;
...
function Shell({ user }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        // Config doc: users/{uid}/config/main
        ...
        if (snap.exists()) {
          ...
          await setDoc(cfgRef, DEFAULT_CONFIG, { merge: true });
          setCfg(DEFAULT_CONFIG);
        }

        // Files collection: users/{uid}/files (order by uploadedAt desc)
        const filesCol = collection(db, "users", uid, "files");
        const q = fsQuery(filesCol, orderBy("uploadedAt", "desc"));
        ...
        unsubFiles = onSnapshot(
          ...
          (err) => {
            console.error("onSnapshot(files) error:", err);
            setDataLoading(false);
          }
        );
      } catch (e) {
